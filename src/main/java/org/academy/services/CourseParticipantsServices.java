...
public class CourseParticipantsServices {
  ...
  public CourseParticipantsDTO addParticipant(ParticipantsRequestDTO participants)
    ...
    CourseEntity course = courseRepository.find("id", participants.courseId()).firstResult();
    if (course == null) {
      throw new NotFounderException("Course not found with ID: " + participants.courseId());
    }
    UserEntity existingUser = userRepository.find("id", participants.userId()).firstResult();
    if (existingUser == null) {
      throw new NotFounderException("User not found with ID: " + participants.userId());
    }
    CourseParticipantsEntity existingParticipant =
        courseParticipantsRepository
            .find("course.id = ?1 and user.id = ?2", course.getId(), existingUser.getId())
            .firstResult();
  ...
  public String removeParticipant(UUID id) throws NotFounderException {
    CourseParticipantsEntity participant =
        courseParticipantsRepository.find("id", id).firstResult();
    if (participant == null) {
